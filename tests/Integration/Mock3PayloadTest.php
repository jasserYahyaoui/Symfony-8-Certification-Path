...
/**
 * Master Plan §10 — Mock 3, Certification difficulty.
 *
 ...
 */
final class Mock3PayloadTest extends TestCase
{
    private const string MOCK = 'mock-3';
    ...
    public function testEveryEligibleQuestionIsAValidationQuestionOnItsOwnItem(): void
    {
        ...
        foreach ($payload['questions'] as $exported) {
            ...
            self::assertSame('hard', $question->difficulty, $question->id->value.' is not at certification difficulty');
    ...
    /**
     * §5 requires Mock 3 to be primarily English, and docs/policy/language-policy.md
     * binds that requirement to the VALIDATION pool. "Primarily" is the floor;
     * the pool happens to clear it completely, which is an outcome rather than
     * a target and is asserted as the floor it actually is.
     */
    public function testTheSittingIsPrimarilyEnglish(): void
    {
        [$payload, $content] = $this->fixture();
        ...
        $english = 0;
        foreach ($payload['questions'] as $exported) {
            if ('en' === $byId[$exported['id']]->language->value) {
                ++$english;
            }
        }
        ...
        self::assertGreaterThan(
            ...
            $english,
            'Mock 3 must be primarily English (§5)',
        );
    }
