...
final class BuildTest extends TestCase
{
    ...
    /**
     * The navigation shows a learner the topic, not the delivery unit: an
     * autogenerated sidebar labels a lot directory from its index page's
     * sidebar_label, so that label must read "05 — Routing" and never "lot-05".
     *
     * The leading number is the recommended revision order. It is not an exam
     * weighting, and the official syllabus publishes no order at all.
     */
    public function testLotNavigationLabelsShowTheTopicNameInRevisionOrder(): void
    {
        $project = Project::locate();
        (new DocsGenerator($project))->generate($project->loadContentSet());
        ...
        $seen = [];
        foreach (glob($project->path('website/docs/courses/lot-*/index.md')) as $index) {
            $lotId = basename(\dirname($index));
            $body = (string) file_get_contents($index);

            self::assertMatchesRegularExpression(
                '/sidebar_label: "\d{2} — /',
                $body,
                $lotId.' must not be labelled with its technical id',
            );
            self::assertStringNotContainsString('sidebar_label: "lot-', $body, $lotId);
            self::assertStringContainsString('sidebar_label: "'.$lots->label($lotId).'"', $body);

            preg_match('/sidebar_position: (\d+)/', $body, $m);
            $seen[$lotId] = (int) ($m[1] ?? 0);
        }

        self::assertNotEmpty($seen, 'no lot index page was generated');

        // The number carries the order, so it must match the registry exactly.
        foreach ($seen as $lotId => $position) {
            self::assertSame($lots->order($lotId), $position, $lotId.' is out of revision order');
        }
    }

    /**
     * A label change must not move a URL. The directory is the lot id, and
     * bookmarks, internal links and the production smoke test all depend on it.
     */
    public function testLotDirectoriesKeepTheirTechnicalIdSoUrlsDoNotMove(): void
    {
        $project = Project::locate();
        (new DocsGenerator($project))->generate($project->loadContentSet());

        $dirs = glob($project->path('website/docs/courses/*'), \GLOB_ONLYDIR);
        self::assertNotEmpty($dirs);

        foreach ($dirs as $dir) {
            self::assertMatchesRegularExpression('/^lot-\d{2}$/', basename($dir));
        }

        // The page the smoke test fetches must still exist at its known path.
        self::assertFileExists($project->path('website/docs/courses/lot-02/status-codes.md'));
    }
