...
1. **Clause 2 rests on B-1, and B-1 has changed shape.** The owner supplied the
   ...
   B-1 is therefore no longer `BLOCKED`, and `SYL-1` and `SYL-2` were fixed in
   PR #68 (merge `8dd3259`), so AUD-01's `FAIL` is lifted; its 26-condition
   completion gate is assessed in the audit report at 25 met, 1 completing.
   What has **not** changed is the ceiling: the PDF is byte-identical
